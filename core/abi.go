...
func DecodeAbi(abiEncoded string) (*abi.ABI, error) {
	...
	if err != nil {
		return nil, fmt.Errorf("error decoding ABI base64 data: %w", err)
	}
	...
	if err != nil {
		return nil, fmt.Errorf("error decompressing ABI zlib data: %w", err)
	}
	defer func() {
		_ = zlibReader.Close()
	}()
	...
	if err != nil {
		return nil, fmt.Errorf("error parsing ABI JSON: %w", err)
	}
