...
func (c *Proposal) GetDAO(mc *multicall.MultiCaller) {
	multicall.AddCall(mc, c.mgr, &c.Details.DAO, "getDAO", c.Details.ID.RawValue)
}

// Get the address of the node that created the proposal
func (c *Proposal) GetProposerAddress(mc *multicall.MultiCaller) {
	multicall.AddCall(mc, c.mgr, &c.Details.ProposerAddress, "getProposer", c.Details.ID.RawValue)
}

// Get the message provided with the proposal
func (c *Proposal) GetMessage(mc *multicall.MultiCaller) {
	multicall.AddCall(mc, c.mgr, &c.Details.Message, "getMessage", c.Details.ID.RawValue)
}

// Get the time the proposal was created
func (c *Proposal) GetCreatedTime(mc *multicall.MultiCaller) {
	multicall.AddCall(mc, c.mgr, &c.Details.CreatedTime.RawValue, "getCreated", c.Details.ID.RawValue)
}

// Get the time the voting window on the proposal started
func (c *Proposal) GetStartTime(mc *multicall.MultiCaller) {
	multicall.AddCall(mc, c.mgr, &c.Details.StartTime.RawValue, "getStart", c.Details.ID.RawValue)
}

// Get the time the voting window on the proposal ended
func (c *Proposal) GetEndTime(mc *multicall.MultiCaller) {
	multicall.AddCall(mc, c.mgr, &c.Details.EndTime.RawValue, "getEnd", c.Details.ID.RawValue)
}

// Get the time the proposal expires
func (c *Proposal) GetExpiryTime(mc *multicall.MultiCaller) {
	multicall.AddCall(mc, c.mgr, &c.Details.ExpiryTime.RawValue, "getExpires", c.Details.ID.RawValue)
}

// Get the number of votes required for the proposal to pass
func (c *Proposal) GetVotesRequired(mc *multicall.MultiCaller) {
	multicall.AddCall(mc, c.mgr, &c.Details.VotesRequired.RawValue, "getVotesRequired", c.Details.ID.RawValue)
}

// Get the number of votes in favor of the proposal
func (c *Proposal) GetVotesFor(mc *multicall.MultiCaller) {
	multicall.AddCall(mc, c.mgr, &c.Details.VotesFor.RawValue, "getVotesFor", c.Details.ID.RawValue)
}

// Get the number of votes against the proposal
func (c *Proposal) GetVotesAgainst(mc *multicall.MultiCaller) {
	multicall.AddCall(mc, c.mgr, &c.Details.VotesAgainst.RawValue, "getVotesAgainst", c.Details.ID.RawValue)
}

// Check if the proposal has been cancelled
func (c *Proposal) GetIsCancelled(mc *multicall.MultiCaller) {
	multicall.AddCall(mc, c.mgr, &c.Details.IsCancelled, "getCancelled", c.Details.ID.RawValue)
}

// Check if the proposal has been executed
func (c *Proposal) GetIsExecuted(mc *multicall.MultiCaller) {
	multicall.AddCall(mc, c.mgr, &c.Details.IsExecuted, "getExecuted", c.Details.ID.RawValue)
}

// Get the proposal's payload
func (c *Proposal) GetPayload(mc *multicall.MultiCaller) {
	multicall.AddCall(mc, c.mgr, &c.Details.Payload, "getPayload", c.Details.ID.RawValue)
}

// Get the proposal's state
func (c *Proposal) GetState(mc *multicall.MultiCaller) {
...
func (c *Proposal) GetMemberSupported(mc *multicall.MultiCaller, out *bool, address common.Address) {
	multicall.AddCall(mc, c.mgr, out, "getReceiptSupported", c.Details.ID.RawValue, address)
}
